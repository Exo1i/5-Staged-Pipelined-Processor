# Member 3 Interface Specification
## ALU + Register File + CCR Module Documentation
...
| `Rdst2` | IN | 3 | Second write destination (SWAP) |
| `WriteData2` | IN | 32 | Second write data (SWAP) |
| `RegWrite2` | IN | 1 | Second write enable (SWAP) |

### Behavior:

- **Read Ports:** Asynchronous (combinational) - data available immediately
- **Write Ports:** Synchronous - writes occur on rising edge of `clk` when `RegWrite = '1'`
- **SWAP Support:** Dual write ports allow two simultaneous register writes
- **Write Conflict:** If both ports write to same register, port 1 (Rdst) takes priority
...
- `Rdst2[2:0]` - Second destination (SWAP)
...
2. **Forwarding Test** (with Member 1 & 2):
   ...
   - Verify: ForwardA/ForwardB signals route correct data
...
✅ Simultaneous reads from same register  
✅ Write conflict (both ports write same register)  
✅ Read-after-write in same cycle  
